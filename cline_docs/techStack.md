...
- **Image Conversion:**
    ...
    - **RAW Decoding (Attempted):** `libraw-wasm` (loaded via local `node_modules`) is being integrated to decode RAW camera files. Currently facing integration issues related to module initialization and API usage.
...
- **Dependency Management:** External libraries (`exif-js`, Leaflet.js) are loaded via CDN links in the HTML. `libraw-wasm` was installed via npm into `1.0.2/node_modules` and is imported as an ES Module in `script.js`.
